...
class Runner:
    def __init__(self, args):
        ...
        # Create env
        self.env = MAgent_Env(self.env_name, self.seed, max_cycles=args.max_step, minimap_mode=True)
        self.env.reset()
    ...
    def evaluate_policy(self, ):
        ...
        for _ in range(self.args.evaluate_times):
            ...
            win_times = {self.key[i]: win_times.get(self.key[i], 0) + 1 if win_tag[self.key[i]]==1 else win_times.get(self.key[i], 0) for i in range(self.N_group)}
    ...
    def run_episode_rmfq(self, evaluate=False):
        ...
        for episode_step in range(self.args.episode_limit):
            ...
            obs_n_next, r, termination, truncations, info, step_done = self.env.step(all_action)  # Take a step   
            ...
            alive, win_tag = self.get_alive_wintag(agent_mask_n_dict_next, next_state)
            ...
            done = True if win_tag[self.key[0]] or step_done else False
        ...
        self.writer.add_scalar('episode_alives_'+'food', alive['food'], self.total_episodes)
    ...
    def get_alive_wintag(self, agent_mask, state):
        alives = {'omnivore':0, 'food':0}
        wintag = {self.key[i]:0 for i in range(self.N_group)}
        for i in range(self.N_group):
            alives[self.key[i]] = sum([1 for item in agent_mask[self.key[i]] if item])
        alives['food'] = state[:,:,1].sum()
        if alives['omnivore'] == 0:
            wintag[self.key[0]] = -1
        elif alives['food'] == 0:
            wintag[self.key[0]] = 1
