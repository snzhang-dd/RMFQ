...
class Runner:
    def __init__(self, args):
        ...
        obs_shape = {self.key[0]:self.env.observation_space(self.env.agents[0]).shape, self.key[1]:self.env.observation_space(self.env.agents[-1]).shape}
        self.args.obs_dim = {self.key[i]:(7, obs_shape[self.key[i]][0], obs_shape[self.key[i]][1]) for i in range(self.N_group)}
        self.args.state_dim = self.env.state_space.shape  # The dimensions of global state space
        self.args.action_dim = {self.key[0]:self.env.action_space(self.env.agents[0]).n, self.key[1]:self.env.action_space(self.env.agents[-1]).n}  # The dimensions of an agent's action space
        ...
        # load mfq 虎的模型
        self.mfq_args = copy.deepcopy(self.args)
        self.mfq_args.target = self.key[1]
        self.mfq_args.algorithm = 'MFQ'
        self.mfq_args.use_recl = False
        self.mfq_args.use_att = False
        self.mfq_args.use_adaptive_k = False
        self.mfq_args.cluster_num = 1
        self.agent_n.update({self.key[1]:MFQ(self.mfq_args)})
        self.agent_n[self.key[1]].load_model('MFQ_tiger_deer/results_711/MFQ/tiger_deer_v4/model/07-24-15-07_cn1_reFalse_attFalse_seed7adap_kFalsel2=0.0001_64MFQmap_size45/tiger', 953036)
        ...
        self.act_mapping = {0: 0, 1: 3, 2: 2, 3: 1, 4: 8, 5: 7, 6: 6, 7:5, 8:4, 9: 11, 11: 9, 10: 10, 12: 12, 13: 15, 15:13, 17:16, 16: 17, 18: 20, 20:18, 14: 14, 19: 19}
        self.mapping_tensor = torch.tensor([self.act_mapping[i] for i in range(max(self.act_mapping.keys())+1)]).to(self.device)
        self.flip_blue = args.train_blue_flip
    ...
    def run(self, ):
        ...
        while self.total_steps < self.args.max_train_steps:
            ...
            if self.replay_buffer[self.key[0]].current_size*500 >= self.args.batch_size and self.train_steps >= self.train_every_steps:
                loss = self.agent_n[self.key[0]].train(self.replay_buffer[self.key[0]])  # Training
                self.writer.add_scalar('mfq_loss_'+self.key[1], loss, self.total_steps)
                self.train_steps = 0
                train_num += 1
    ...
    def run_episode_rmfq(self, evaluate=False):
        ...
        last_onehot_a_n = {self.key[i]:np.zeros((self.args.N[self.key[i]], self.args.action_dim[self.key[i]])) for i in range(self.N_group)}  # Last actions of N agents(one-hot)
        a_n={}
        ...
        # former_act_prob = {self.key[i]:[np.zeros((self.args.action_dim[self.key[i]]), dtype=np.float32) for _ in range(cluster_num)] for i in range(self.N_group)}
        former_act_prob = {self.key[0]:[np.zeros((self.args.action_dim[self.key[0]]), dtype=np.float32) for _ in range(cluster_num)]}
        former_act_prob.update({self.key[1]:[np.zeros((self.args.action_dim[self.key[1]]), dtype=np.float32) for _ in range(1)]})
        # if self.args.use_att:
        mean_act_list = copy.deepcopy(former_act_prob)
        center = self.center
        role_embedding_n = {}
        for i in range(self.N_group):
            if self.args.use_recl and self.args.algorithm=='RMFQ' and i==0:
        ...
        for episode_step in range(self.args.episode_limit):
            ...
            for i in range(self.N_group):
                ...
                epsilon1 = epsilon if i==0 else 0.1
                if i==0:
            ...
            for j in range(len(agent_mask_n_next)):
                if not agent_mask_n_next[j]:
                    if self.key[0] in self.agents[j]:
                        k=0
                    else:
                        k=1
                    obs_n_next_padding[self.agents[j]]=np.zeros([self.args.obs_dim[self.key[k]][1], self.args.obs_dim[self.key[k]][2], 9], dtype=np.float32)
            ...
            # r_avg = {self.key[i]:r_total[self.key[i]]/alive[self.key[i]] for i in range(self.N_group)}
            ...
            if not evaluate:
                ...
                last_onehot_a_n = {self.key[i]:np.eye(self.args.action_dim[self.key[i]])[a_n[self.key[i]]] for i in range(self.N_group)}  # Convert actions to one-hot vectors
            ...
            if self.flip_blue:
                obs_blue = self.flip_blue_obs(obs_n_array[self.key[1]], state, position[self.key[1]])
                obs_n_array[self.key[1]] = obs_blue
    ...
    def get_mean_act(self, former_act_prob, obs, role_global, act_global, mask, state, position):
        for i in range(self.N_group):
            # if i == 1:
            #     obs_ = self.flip_blue_obs(obs[self.key[i]], state, position[self.key[i]])
            # else:
            obs_ = obs[self.key[i]]
    ...
    def flip_blue_obs(self, obs_n, state, position):
        obs_blue = np.flip(obs_n, axis=-1).copy()
        position = torch.FloatTensor(position).to(self.device).unsqueeze(0)
        state = torch.FloatTensor(state).to(self.device).unsqueeze(0).expand(1, self.args.N[self.key[1]], self.args.state_dim[0], self.args.state_dim[1], self.args.state_dim[2])
    ...
    def flip_blue_act(self, act_n):
        return self.mapping_tensor[act_n]
