...
class RMFQ_NET():
    ...
    def act(self, obs, role_embedding, mean_action, eps):
        ...
        if np.random.uniform() < eps:
            ...
            mean_action = mean_action.reshape(-1, cluster_num, self.action_dim)
            _, mean_action_desc = get_mean_action_description(mean_action)
            q_values = self.eval_net(obs, role_embedding, mean_action_desc)
            # predict = F.softmax(q_values / eps, dim=-1)
            # actions = torch.multinomial(predict, 1).squeeze()
    ...
    def calc_target_q_nornn(self, obs_next, rewards, dones, mean_act_next, act):
        # with torch.no_grad():
        act_one_hot = F.one_hot(act.to(torch.int64), num_classes=self.action_dim).reshape(-1, self.action_dim)
    ...
    def update_q_nornn(self, obs, act, next_obs, reward, done, mask, mean_action:torch.FloatTensor, next_mean_action, last_act):
        ...
        if not self.args.use_att:
            mean_action = mean_action.reshape(-1, cluster_num, self.action_dim)
            next_mean_action = next_mean_action.reshape(-1, cluster_num, self.action_dim)
            _, mean_action_desc = get_mean_action_description(mean_action)
            _, next_mean_action_desc = get_mean_action_description(next_mean_action)
        else:
            next_mean_action_desc = {}
            mean_action = mean_action.reshape(-1, cluster_num, self.action_dim)
            next_mean_action['eval'] = next_mean_action['eval'].reshape(-1, cluster_num, self.action_dim)
            next_mean_action['target'] = next_mean_action['target'].reshape(-1, cluster_num, self.action_dim)
            _, mean_action_desc = get_mean_action_description(mean_action)
    ...
    def cal_att_weights_mean_acts(self, obs, role_global, act_global, mask, key = 'eval'):
        ...
        if self.args.use_att:
            ...
            act = F.one_hot(act_global, self.action_dim).to(torch.float32).unsqueeze(1).expand(-1, cluster_num, -1, -1)
